...
def entropic_gromov_wasserstein_change(Xs,Ts,Xt,Tt,C1, C2, p, q, loss_fun, epsilon,leimuda,
                                max_iter=30, tol=1e-9, verbose=False, log=False):
    """
    Returns the gromov-wasserstein transport between (C1,p) and (C2,q)
    ...
    p_regularization = np.zeros((2,ns))
    ns_1 = sum(Ts)[0]
    ns_0 = ns - ns_1
    q_regularization = np.zeros((2,nt))
    nt_1 = sum(Tt)[0]
    nt_0 = nt - nt_1

    n_all = ns_0 + ns_1 + nt_0 + nt_1
    print("ns_0",ns_0,"ns_1",ns_1,"nt_0",nt_0,"nt_1",nt_1)

    for p_i in range(ns):
        if(Ts[p_i] == 0):
            #p_regularization[0][p_i] = 1 / (ns_0 * nt * ns * nt)
            p_regularization[0][p_i] = 1 / ns_0
        else:
            #p_regularization[1][p_i] = 1 / (ns_1 * nt * ns * nt)
            p_regularization[1][p_i] = 1 / ns_1  

    for q_i in range(nt):
        if(Tt[q_i] == 0):
            #q_regularization[0][q_i] = 1 / (nt_0 * ns * nt * ns)
            q_regularization[0][q_i] = 1 / nt_0
        else:
            #q_regularization[1][q_i] = 1 / (nt_1 * ns * nt * ns)
            q_regularization[1][q_i] = 1 / nt_1 
    ...
    while (err > tol and cpt < max_iter):
        ...
        # add one regularization
        regu1 = 2 * ns * ns * p_regularization.T.dot(p_regularization).dot(T).dot(Xt).dot(Xt.T) 
        regu2 = 2 * ns *  p_regularization.T.dot(q_regularization).dot(Xt).dot(Xt.T)
        regu = regu1 - regu2


        # regu1_ = ns *(p_regularization.dot(T).dot(Xt))
        # regu2_ = q_regularization.dot(Xt)
        # regu_ = regu1_ - regu2_
        # regu_ = np.linalg.norm(regu_, ord='fro')

        #print("before,after:",cpt)
        #print(tens)
        #print(regu)

        #除法
        #regu_multiple = tens / regu
        #print(regu_multiple)
        #print(np.mean(regu_multiple))

        #tens = tens + regu / n_all
        tens = tens + regu * leimuda 
